...
## Fisher++ (Fisher/Spectral Analysis)

If you aren't sure which modules to target or what rank to use, Side-Step provides the `fisher` subcommand. It runs a forward/backward pass to generate a rank distribution map that the training script will automatically use.

```bash
uv run python train.py fisher \
    --checkpoint-dir models/stable-audio-open-1.0 \
    --dataset-dir ./my_preprocessed_dataset \
    --target-average-rank 64 \
    --max-rank 256
```

This creates a `preprocessing_pp.json` in your dataset directory. When you run `train.py fixed` with that dataset, it will automatically detect the map and use it to assign varying ranks to different layers. Useful for:
