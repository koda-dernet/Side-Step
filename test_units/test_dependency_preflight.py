...
class TestDependencyPromptFlow(unittest.TestCase):
    ...
    def test_decline_install_keeps_dependency_unresolved(self) -> None:
        with (
            patch("sidestep_engine.ui.dependency_check._has_module", return_value=False),
            patch("sidestep_engine.ui.dependency_check._ask_yes_no", return_value=False),
        ):
    ...
    def test_accept_install_resolves_dependency(self) -> None:
        with (
            patch(
                "sidestep_engine.ui.dependency_check._has_module",
                side_effect=[False, True],
            ),
            patch("sidestep_engine.ui.dependency_check._ask_yes_no", return_value=True),
            patch("sidestep_engine.ui.dependency_check._install_dep", return_value=True),
        ):
            unresolved = ensure_optional_dependencies(
                [self.dep], interactive=True, allow_install_prompt=True
            )
    ...
    def test_noninteractive_mode_does_not_prompt(self) -> None:
        with (
            patch("sidestep_engine.ui.dependency_check._has_module", return_value=False),
            patch("sidestep_engine.ui.dependency_check._ask_yes_no") as ask_mock,
        ):
            unresolved = ensure_optional_dependencies(
