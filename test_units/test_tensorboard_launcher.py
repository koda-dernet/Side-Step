...
class TestTensorboardLauncher(unittest.TestCase):
    ...
    def test_noninteractive_no_skip_does_not_autolaunch(self) -> None:
        """CI / automation: no skip_prompt, no TTY → never launch."""
    ...
    def test_skip_prompt_overrides_noninteractive(self) -> None:
        """GUI subprocess: -y flag (skip_prompt=True) but stdin piped → still launch."""
        decision = should_launch_tensorboard(
            os.path.join(_TMP, "runs"),
            default=True,
            skip_prompt=True,
    ...
    def test_skip_prompt_accepts_default(self) -> None:
    ...
    @patch("sidestep_engine.ui.tensorboard_launcher.subprocess.Popen")
    ...
    @patch(
        "sidestep_engine.ui.tensorboard_launcher.subprocess.Popen",
